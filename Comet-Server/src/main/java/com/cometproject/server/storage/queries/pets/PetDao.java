...
public class PetDao {
    ...
    public static void savePetsBatch(final Set<IPetData> pets) {
        Connection sqlConnection = null;
        PreparedStatement preparedStatement = null;
        try {
            sqlConnection = SqlHelper.getConnection();
            preparedStatement = SqlHelper.prepare("UPDATE pet_data SET scratches = ?, level = ?, happiness = ?, experience = ?, energy = ?, hunger = ?, extra_data = ?, x = ?, y = ?, saddled = ?, hair_style = ?, hair_colour = ?, any_rider = ?, race_id = ?, room_id = ?, z = ?, birthday = ? WHERE id = ?", sqlConnection);
            for (IPetData data : pets) {
                ...
                preparedStatement.setInt(17, data.getBirthday());
                preparedStatement.setInt(18, data.getId());
