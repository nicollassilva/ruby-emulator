...
public class RoomBotDao {
    public static List<IBotData> getBotsByRoomId(int roomId) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                final boolean isRandomMessages = resultSet.getBoolean("chat_random");
                ...
                PlayerBotData botData = new PlayerBotData(id, username, motto, figure, gender, ownerName, ownerId, messages, automaticChat, isRandomMessages, chatDelay, botType, mode, storedData);
    ...
    public static void saveData(BotData data) {
        ...
        try {
            ...
            preparedStatement = SqlHelper.prepare("UPDATE bots SET figure = ?, gender = ?, motto = ?, name = ?, messages = ?, automatic_chat = ?, chat_delay = ?, mode = ?, data = ?, chat_random = ? WHERE id = ?", sqlConnection);
            ...
            preparedStatement.setString(10, data.isRandomMessages() ? "1" : "0");

            preparedStatement.setInt(11, data.getId());
