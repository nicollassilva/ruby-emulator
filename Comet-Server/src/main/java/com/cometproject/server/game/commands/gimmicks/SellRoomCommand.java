...
public class SellRoomCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        for (final PlayerEntity playerEntity : room.getEntities().getPlayerEntities())
            playerEntity.getPlayer().getSession().send(new WhisperMessageComposer(playerEntity.getId(), "Este quarto " + (!isSelling ? "já não está à venda!" : "está à venda por " + value + " diamantes! E apenas o usuário " + PlayerDao.getUsernameByPlayerId(room.getData().getRoomBuyer()) + " pode comprá-lo!")));
    }
