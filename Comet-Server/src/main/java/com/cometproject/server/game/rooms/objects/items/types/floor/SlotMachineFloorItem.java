...
public class SlotMachineFloorItem extends RoomItemFloor {
    ...
    public boolean onInteract(RoomEntity entity, int requestData, boolean isWiredTrigger) {
        ...
        if(random1 == random2 && random2 == random3) {
            ...
            playerEntity.getPlayer().getSession().send(new NotificationMessageComposer(image, "Você ganhou %q Diamantes com o caça-níqueis.\n\n(%b x %m)"
            ...
            playerEntity.getPlayer().getSession().send(
                    new NotificationMessageComposer("slot_loss", "Infelizmente a sequência não foi igual e você perdeu %amount% diamantes.".replace("%amount%", playerEntity.getBetAmount() + ""))
            );
        }
        ...
        playerEntity.getPlayer().getSession().send(new TalkMessageComposer(entity.getId(), "Você tirou " + rand1 + " " + rand2 + " " + rand3 + ".", ChatEmotion.NONE, 34));
