...
public class SlotMachineCreditsFloorItem extends RoomItemFloor {
    ...
    public boolean onInteract(RoomEntity entity, int requestData, boolean isWiredTrigger) {
        ...
        if(random1 == random2 && random2 == random3) {
            ...
            playerEntity.getPlayer().getSession().send(new NotificationMessageComposer(image, "Você ganhou %q Moedas com o caça-níqueis.\n\n(%b x %m)"
            ...
            playerEntity.getPlayer().getSession().send(
                    new NotificationMessageComposer("slot_loss", "Infelizmente a sequência não foi igual e você perdeu %amount% moedas.".replace("%amount%", playerEntity.getBetAmount() + ""))
            );
        }
