...
public class FishingPoolFloorItem
        ...
        if (!isWiredTrigger) {
            ...
            if (this.incomingEntity != null) {
                if (this.getRoom().getEntities().getPlayerEntities().contains(this.incomingEntity)) {
                    player.sendBubble("fishing_in_use", "Este punto de pesca se encuentra en uso, prueba una vez se haya desocupado.");
                    return false;
                }
                this.disposeItem();
            } else {
    ...
    private void startFishing() {
        this.incomingEntity.getPlayer().getSession().send(new WhisperMessageComposer(this.incomingEntity.getId(), "<i>Lanzas la ca\u00f1a junto a la carnada al punto de pesca...</i>", 1));
    ...
    private void removeFishingReagents(boolean isFishingFail) {
        if (isFishingFail) {
            this.incomingEntity.getPlayer().getSession().send(new WhisperMessageComposer(this.incomingEntity.getId(), "<i>Tu ca\u00f1a acaba de romperse tras una batalla con el pez que mordi\u00f3 el anzuelo...</i>", 1));
            this.disposeItem();
        } else {
