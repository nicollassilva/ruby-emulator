...
public class RoomBotComponent {
    ...
    public BotEntity addBot(IBotData bot, int x, int y, double height) {
        ...
        BotData botData = new PlayerBotData(bot.getId(), name, bot.getMotto(), bot.getFigure(), bot.getGender(), bot.getOwnerName(), bot.getOwnerId(), "[]", bot.isAutomaticChat(), bot.isRandomMessages(), bot.getChatDelay(), bot.getBotType(), bot.getMode(), null);
        ...
        botEntity.say("Beep beep boop boop!");
        this.getRoom().getEntities().addEntity(botEntity);
        return botEntity;
    }
