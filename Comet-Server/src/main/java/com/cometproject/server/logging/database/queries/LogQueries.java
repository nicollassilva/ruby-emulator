...
public class LogQueries {
    ...
    public static void putRoomEntryBatch(final Set<RoomVisitLogEntry> entries) {
        Connection sqlConnection = null;
        PreparedStatement preparedStatement = null;
        try {
            sqlConnection = SqlHelper.getConnection();
            preparedStatement = SqlHelper.prepare("INSERT into player_room_visits (`player_id`, `room_id`, `time_enter`) VALUES(?, ?, ?);", sqlConnection, true);
            for (RoomVisitLogEntry entry : entries) {
                preparedStatement.setInt(1, entry.getPlayerId());
                preparedStatement.setInt(2, entry.getRoomId());
                preparedStatement.setInt(3, entry.getEntryTime());
    ...
    public static void updateRoomEntryBatch(final Set<RoomVisitLogEntry> entries) {
        Connection sqlConnection = null;
        PreparedStatement preparedStatement = null;
        try {
            sqlConnection = SqlHelper.getConnection();
            preparedStatement = SqlHelper.prepare("UPDATE player_room_visits SET time_exit = ? WHERE player_id = ? AND room_id = ? AND time_enter = ?", sqlConnection);
            for (RoomVisitLogEntry entry : entries) {
                preparedStatement.setInt(1, entry.getExitTime());
                preparedStatement.setInt(2, entry.getPlayerId());
                preparedStatement.setInt(3, entry.getRoomId());
                preparedStatement.setInt(4, entry.getEntryTime());
                preparedStatement.addBatch();
            }
            preparedStatement.executeBatch();
        } catch (SQLException e) {
    ...
    public static void putEntryBatch(final Set<AbstractLogEntry> entries) {
        Connection sqlConnection = null;
        PreparedStatement preparedStatement = null;

        try {
            sqlConnection = SqlHelper.getConnection();
            preparedStatement = SqlHelper.prepare("INSERT into logs (`type`, `room_id`, `user_id`, `data`, `timestamp`) VALUES(?, ?, ?, ?, ?);", sqlConnection);
