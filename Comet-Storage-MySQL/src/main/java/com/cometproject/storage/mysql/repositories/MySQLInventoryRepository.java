...
public class MySQLInventoryRepository extends MySQLRepository implements IInventoryRepository {
    ...
    public void getInventoryByPlayerId(int playerId, Consumer<List<PlayerItem>> itemConsumer) {
        ...
        select("SELECT i.*, ltd.limited_id, ltd.limited_total FROM items i LEFT JOIN items_limited_edition ltd ON ltd.item_id = i.id WHERE room_id = 0 AND user_id = ? ORDER by id DESC;", (data) -> items.add(this.buildItem(data)), playerId);
