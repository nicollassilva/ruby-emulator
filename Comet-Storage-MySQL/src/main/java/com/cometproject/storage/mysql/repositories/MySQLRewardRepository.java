...
public class MySQLRewardRepository extends MySQLRepository implements IRewardRepository {
    ...
    public void getActiveRewards(Consumer<Map<String, RewardData>> consumer) {
        ...
        select("SELECT code, badge, vip_points, seasonal_points FROM player_rewards WHERE active = ?", (data) -> rewards.put(data.readString("code"), new RewardData(data.readString("code"), data.readString("badge"), data.readInteger("vip_points"), data.readInteger("seasonal_points"))), "1");
